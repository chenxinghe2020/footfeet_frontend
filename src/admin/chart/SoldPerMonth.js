    ...
    useEffect(()=>{
        loginState.orders&&loginState.orders.forEach(order=>{
            order.purchases.forEach((orderProduct)=>{
                data[(new Date(order.date).getMonth())].total+=(+orderProduct.qty);
                if(orderProduct.product.brand.toLowerCase()==='nike'){
                    data[(new Date(order.date).getMonth())].nike+=(+orderProduct.qty);
                }
                if(orderProduct.product.brand.toLowerCase()==='adidas'){
                    data[(new Date(order.date).getMonth())].adidas+=(+orderProduct.qty);
                }
                if(orderProduct.product.brand.toLowerCase()==='jordan'){
                    data[(new Date(order.date).getMonth())].jordan+=(+orderProduct.qty);
                }
                if(orderProduct.product.brand.toLowerCase()==='new balance'){
                    data[(new Date(order.date).getMonth())]["new balance"]+=(+orderProduct.qty);
                }
            })
            setData(data);
